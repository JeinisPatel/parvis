...
"""
    ...
    def _build_docx() -> bytes:
        """Generate a professional Word document audit report with logo watermark."""
        import io
        ...
        logo_path = next((p for p in ["ethical_ai_logo.png","parvis/ethical_ai_logo.png",
                           "/mount/src/parvis/ethical_ai_logo.png"] if __import__("os").path.exists(p)), None)
        header = section.header
        hp = header.paragraphs[0]
        hp.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        if logo_path:
            run_h = hp.add_run()
            run_h.add_picture(logo_path, width=Inches(0.55))
        # Header rule
        ...
        # Insert via header with large, centered, low-opacity image
        if logo_path:
            wm_header = section.header
            # Add a second paragraph for watermark positioning
            wm_p = OxmlElement("w:p")
            wm_pPr = OxmlElement("w:pPr")
            wm_jc = OxmlElement("w:jc")
            wm_jc.set(qn("w:val"), "center")
            wm_pPr.append(wm_jc)
            # Spacing push-down
            wm_sp = OxmlElement("w:spacing")
            wm_sp.set(qn("w:before"), "3000")
            wm_pPr.append(wm_sp)
            wm_p.append(wm_pPr)
            wm_r = OxmlElement("w:r")
            wm_rPr = OxmlElement("w:rPr")
            wm_r.append(wm_rPr)
            wm_p.append(wm_r)

        # ── Title block ────────────────────────────────────────────────────────
        def add_rule(doc, color="1B2A4A"):
            p = doc.add_paragraph()
            pPr = p.paragraph_format._element.get_or_add_pPr()
            pBdr = OxmlElement("w:pBdr")
            bottom = OxmlElement("w:bottom")
            bottom.set(qn("w:val"), "single")
        ...
        def add_heading(doc, text, level=1, color="1B2A4A"):
            p = doc.add_paragraph()
        ...
        def add_field(doc, label, value, label_color="185FA5"):
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(2)
            p.paragraph_format.space_after  = Pt(2)
        ...
        tr = tp.add_run("P.A.R.V.I.S")
        tr.bold = True; tr.font.size = Pt(28)
        tr.font.color.rgb = RGBColor.from_string("1B2A4A")
        tr.font.name = "Arial"
        ...
        sr.italic = True; sr.font.size = Pt(11); sr.font.name = "Arial"
        ...
        for part, col in [
            ...
            (f"  ·  {datetime.now().strftime('%d %B %Y')}", "555555"),
        ]:
            ir = ip.add_run(part)
            ir.font.size = Pt(9); ir.font.name = "Arial"
            ir.font.color.rgb = RGBColor.from_string(col)

        add_rule(doc)

        # ── Inference output ────────────────────────────────────────────────────
        add_heading(doc, "INFERENCE OUTPUT", color="1B2A4A")
        risk_col = "A32D2D" if da >= 0.55 else "BA7517" if da >= 0.35 else "3B6D11"
        rp = doc.add_paragraph()
        rp.paragraph_format.space_before = Pt(4)
        rr1 = rp.add_run("Node 20 — DO Designation Risk:  ")
        rr1.bold = True; rr1.font.size = Pt(13); rr1.font.name = "Arial"
        rr2 = rp.add_run(f"{da*100:.2f}%  [{bla.upper()}]")
        rr2.bold = True; rr2.font.size = Pt(13); rr2.font.name = "Arial"
        ...
        # ── Doctrinal framework ─────────────────────────────────────────────────
        add_heading(doc, "DOCTRINAL FRAMEWORK — THE TETRAD", color="185FA5")
        for cite in [
            "R v Gladue [1999] 1 SCR 688",
            "R v Ipeelee [2012] SCC 13",
            f"R v Morris 2021 ONCA 680 (para 97) — Framework: {st.session_state.scefw.upper()} · Connection: {st.session_state.conn.upper()} · Multiplier: {mx:.2f}",
            "R v Ellis 2022 BCCA 278",
            "Ewert v Canada [2018] SCC 30",
            "R v Boutilier 2017 SCC 64",
            "R v Natomagan 2022 ABCA 48",
        ...
        # ── Gladue factors ──────────────────────────────────────────────────────
        add_heading(doc, "GLADUE FACTORS", color="1B5E20")
        if cG:
            for f in cG:
                add_field(doc, f"[✓] {f['l']}", f"Node {f['n']}  (+{f['w']*100:.0f}%)", "1B5E20")
        else:
            add_body(doc, "No Gladue factors selected.")

        # ── Morris / Ellis SCE ──────────────────────────────────────────────────
        add_heading(doc, "MORRIS / ELLIS SOCIAL CONTEXT EVIDENCE", color="185FA5")
        if cS:
            for f in cS:
                add_field(doc, f"[✓] {f['l']}",
                    f"Node {f['n']}  (+{f['w']*mx*100:.1f}% after connection weight {mx:.2f})", "185FA5")
        else:
            add_body(doc, "No Morris/Ellis SCE factors selected.")

        # ── Criminal record ─────────────────────────────────────────────────────
        if cr_rec:
            add_heading(doc, "CALIBRATED CRIMINAL RECORD", color="A32D2D")
            esc_i = st.session_state.cr_doc_adj.get("escalation", {})
            add_field(doc, "Pattern (Boutilier)",
                f"{esc_i.get('pattern','—').title()} — {esc_i.get('note','')}", "A32D2D")
            ...
            for ci, htext in enumerate(["Year","Offence","Seriousness","Cal. Weight","Distortions"]):
                hdr[ci].text = htext
                hdr[ci].paragraphs[0].runs[0].bold = True
                hdr[ci].paragraphs[0].runs[0].font.size = Pt(9)
                hdr[ci].paragraphs[0].runs[0].font.name = "Arial"
            for e in cr_rec:
                row = tbl.add_row().cells
                dists = []
                if e["adj"]["bail"]>0.1:   dists.append(f"Bail {e['adj']['bail']:.2f}")
                if e["adj"]["police"]>0.1: dists.append(f"Police {e['adj']['police']:.2f}")
                if e["adj"]["gladue"]>0.1: dists.append(f"Gladue {e['adj']['gladue']:.2f}")
                if e["adj"]["time"]>0.1:   dists.append(f"Time {e['adj']['time']:.2f}")
                if e.get("gang"):          dists.append("Gang")
                for ci, val in enumerate([
                    str(e["year"]), e["offence"],
                    ...
                    row[ci].paragraphs[0].runs[0].font.name = "Arial"
            doc.add_paragraph()

        # ── Risk posteriors table ───────────────────────────────────────────────
        add_heading(doc, "RISK FACTOR POSTERIORS", color="A32D2D")
        ...
        for ci, ht in enumerate(["Node","Factor","P(High)%"]):
            ...
            tbl2.rows[0].cells[ci].paragraphs[0].runs[0].font.size = Pt(9)
        for nid in NODE_META:
            if NODE_META[nid]["type"] == "risk" and nid != 20:
        ...
        # ── Distortion corrections table ───────────────────────────────────────
        add_heading(doc, "SYSTEMIC DISTORTION CORRECTIONS", color="185FA5")
        tbl3 = doc.add_table(rows=1, cols=3)
        tbl3.style = "Table Grid"
        for ci, ht in enumerate(["Node","Factor","P(High)%"]):
            tbl3.rows[0].cells[ci].text = ht
            tbl3.rows[0].cells[ci].paragraphs[0].runs[0].bold = True
            tbl3.rows[0].cells[ci].paragraphs[0].runs[0].font.size = Pt(9)
        for nid in NODE_META:
            if NODE_META[nid]["type"] not in ("risk","output") and nid != 20:
        ...
        # ── QBism diagnostics ──────────────────────────────────────────────────
        if st.session_state.qdiags:
            add_heading(doc, "QBISM DIAGNOSTIC REPORT (APPENDIX Q)", color="534AB7")
            qd = st.session_state.qdiags
            add_field(doc, "Overall flag", qd.get("overall_flag","—").upper(), "534AB7")
            add_field(doc, "Superposition index", f"{qd.get('superposition_index','—')} / 1.0", "534AB7")
            add_body(doc, qd.get("summary",""))
            for check, label in [
                ...
                sev_col = "A32D2D" if sev=="HIGH" else "BA7517" if sev=="MODERATE" else "3B6D11"
                add_field(doc, label, f"{sev} {'⚠️' if d.get('flagged') else '✓'}", sev_col)
        ...
        for part, col in [
            ...
            ("© Jeinis Patel, PhD Candidate and Barrister  ·  Ethical AI Initiative  ·  University of London", "555555"),
        ]:
            fr = fp.add_run(part)
            fr.font.size = Pt(8); fr.font.name = "Arial"; fr.italic = True
        ...
        ftr.font.size = Pt(8); ftr.font.name = "Arial"
    ...
    def _build_pdf() -> bytes:
        """Generate a clean PDF audit report using fpdf2."""
        ...
        class PARVISReport(FPDF):
            def header(self):
                ...
                if logo_path:
                    # Watermark — large, centred, very low opacity
                    self.set_xy(60, 80)
                    with self.local_context(fill_opacity=0.06, stroke_opacity=0.06):
                        self.image(logo_path, x=60, y=80, w=90)
                # Header bar
                self.set_fill_color(27, 42, 74)
                self.rect(0, 0, 210, 1.5, "F")
                if logo_path:
                    self.image(logo_path, x=175, y=4, w=14)
                self.set_xy(14, 4)
                self.set_font("Helvetica", "B", 8)
                self.set_text_color(27, 42, 74)
                self.cell(0, 8, "P.A.R.V.I.S  ·  Audit Report", ln=True)
            ...
            def footer(self):
                ...
                self.set_fill_color(27, 42, 74)
                self.rect(0, self.h-1.5, 210, 1.5, "F")
                self.cell(0, 10,
                    f"PARVIS · Research use only · NOT for deployment in live proceedings · "
                    f"© Jeinis Patel · Page {self.page_no()}", align="C")
        ...
        pdf.cell(0, 12, "P.A.R.V.I.S", align="C", ln=True)
        pdf.set_font("Helvetica", "I", 10)
        pdf.set_text_color(80, 80, 80)
        pdf.cell(0, 6, "Probabilistic and Analytical Reasoning Virtual Intelligence System", align="C", ln=True)
        pdf.ln(2)
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(163, 45, 45)
        pdf.cell(70, 5, "Jeinis Patel, PhD Candidate and Barrister", align="C")
        pdf.set_text_color(24, 95, 165)
        pdf.cell(60, 5, "University of London", align="C")
        pdf.set_text_color(27, 94, 32)
        pdf.cell(45, 5, "Ethical AI Initiative", align="C", ln=True)
        pdf.ln(2)
        ...
        def body_line(text, indent=True):
            pdf.set_font("Helvetica", "", 9)
        ...
        def field_line(label, value, lr=24, lg=95, lb=165):
            pdf.set_x(24)
        ...
        pdf.set_x(24)
        pdf.multi_cell(0, 5,
            "This figure models DESIGNATION RISK — not intrinsic dangerousness. "
            "Distortion node corrections reduce the evidentiary weight of upstream risk signals.", ln=True)
        pdf.ln(3)
        ...
        for cite in [
            "R v Gladue [1999] 1 SCR 688",
            "R v Ipeelee [2012] SCC 13",
            f"R v Morris 2021 ONCA 680 — {st.session_state.scefw.upper()} · Connection: {st.session_state.conn} · ×{mx:.2f}",
            "R v Ellis 2022 BCCA 278",
            "Ewert v Canada [2018] SCC 30",
            "R v Boutilier 2017 SCC 64",
            "R v Natomagan 2022 ABCA 48",
        ]:
            pdf.set_x(24); pdf.set_font("Helvetica","I",9)
            pdf.set_text_color(50,50,50)
            pdf.cell(4,5,"•"); pdf.cell(0,5,cite,ln=True)
        pdf.ln(3)
        ...
        if cG:
            for f in cG:
                field_line(f['l'], f"Node {f['n']}  (+{f['w']*100:.0f}%)", 27,94,32)
        else:
            body_line("No Gladue factors selected.")
        pdf.ln(2)
        ...
        if cS:
            for f in cS:
                field_line(f['l'], f"Node {f['n']}  (+{f['w']*mx*100:.1f}%)", 24,95,165)
        else:
            body_line("No Morris/Ellis SCE factors selected.")
        pdf.ln(2)
        ...
        if cr_rec:
            ...
            esc_i = st.session_state.cr_doc_adj.get("escalation", {})
            field_line("Pattern (Boutilier)", esc_i.get("pattern","—").title(), 163,45,45)
            body_line(esc_i.get("note",""))
            pdf.ln(1)
            # Table header
            col_ws = [15, 75, 40, 25, 20]
            ...
            for ei, e in enumerate(cr_rec):
                pdf.set_fill_color(248,248,248 if ei%2==0 else 255)
                pdf.set_text_color(50,50,50)
                pdf.set_x(18)
                for txt, w in zip([
                    str(e["year"]),
                    e["offence"][:40],
                    e.get("seriousness_label","—")[:20],
                    f"{e['cal_weight']*100:.0f}%",
                    "Yes" if e.get("gang") else "No",
                ], col_ws):
                    pdf.cell(w, 5, txt, border=0, fill=True)
                pdf.ln()
            pdf.ln(3)
        ...
        for nid in NODE_META:
            if NODE_META[nid]["type"]=="risk" and nid!=20:
                ...
                field_line(f"N{nid}  {NODE_META[nid]['name']}", f"{pval:.1f}%", *col)
        pdf.ln(2)
        ...
        for nid in NODE_META:
            if NODE_META[nid]["type"] not in ("risk","output") and nid!=20:
                pval = Pa.get(nid,0.5)*100
                field_line(f"N{nid}  {NODE_META[nid]['name']}", f"{pval:.1f}%", 24,95,165)
        pdf.ln(2)
        ...
        if st.session_state.qdiags:
            ...
            qd = st.session_state.qdiags
            field_line("Overall flag", qd.get("overall_flag","—").upper(), 83,74,183)
            field_line("Superposition index", f"{qd.get('superposition_index','—')} / 1.0", 83,74,183)
            body_line(qd.get("summary",""))
        pdf.ln(4)
        ...
        pdf.cell(0, 5,
            "Research use only. Not for deployment in live proceedings. "
